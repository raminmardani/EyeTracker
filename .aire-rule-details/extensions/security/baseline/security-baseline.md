...
## Overview
**ALWAYS ENFORCED — no opt-in.** This extension has no `.opt-in.md` file, so per the extensions loading convention it is loaded at workflow start and enforced for every project. Never ask the user whether security rules should apply, and ignore any legacy `Security Baseline | No` row in `## Extension Configuration` (older projects captured before this became mandatory) — enforcement cannot be disabled.

These security rules are MANDATORY cross-cutting constraints that apply across all aire phases. They are not optional guidance — they are hard constraints that stages MUST enforce when generating questions, producing design artifacts, generating code, and presenting completion messages.
...
4. The finding MUST be logged in `.spec/aire-docs/audit.md` with the SECURITY rule ID, description, and stage context
...
- **Log injection prevention**: Untrusted input written to logs MUST be sanitized (strip or encode CR/LF and control characters) to prevent log forging and poisoning of the audit trail
...
- Untrusted values are CR/LF-sanitized before being written to logs
...
- **SSRF prevention**: Any endpoint that fetches a user-supplied URL, hostname, or resource reference MUST validate the target against an allowlist of permitted hosts/schemes, reject requests to internal/link-local/metadata addresses (e.g., `169.254.169.254`, `127.0.0.0/8`, RFC 1918 ranges), and MUST NOT follow redirects to disallowed destinations
- **File uploads**: Any endpoint that accepts file uploads MUST validate the file by content signature (magic bytes) — not just the client-supplied `Content-Type` or extension — enforce an extension/MIME allowlist, cap file size, generate server-side random filenames, and store uploads outside the web root (or in isolated object storage). Uploaded content MUST NOT be served back with an executable content type.
...
- Endpoints that fetch remote resources validate the target against a host/scheme allowlist and block internal/metadata addresses
- File upload handlers validate content signatures, enforce an allowlist, randomize stored filenames, and store outside the web root
...
- **Token validation**: JWTs or session tokens MUST be validated server-side on every request (signature, expiration, audience, issuer). JWT signature algorithms MUST be pinned server-side — never accept the `alg` header from the token, and reject `none`
- **CSRF protection**: Every state-changing request (POST/PUT/PATCH/DELETE) authenticated via cookies MUST be protected against cross-site request forgery — use anti-CSRF tokens (synchronizer or double-submit pattern) and/or `SameSite=Strict`/`Lax` session cookies. Endpoints authenticated exclusively via a non-cookie bearer token are exempt
- **Mass assignment protection**: Request bodies MUST NOT be bound directly to domain/persistence objects that contain sensitive or privilege-bearing fields (e.g., `role`, `isAdmin`, `balance`, `ownerId`). Use explicit allowlists (DTOs, `fields=[...]`, strong parameters) so clients cannot set fields they are not authorized to modify
- **Open redirect protection**: User-supplied values MUST NOT be used directly as redirect or forward destinations. Use a server-side mapping (ID → URL) or validate the target against a trusted-domain allowlist
...
- Token validation occurs server-side on every request (not just at login), and JWT algorithms are pinned (no `alg: none`, no header-controlled algorithm)
- Cookie-authenticated state-changing endpoints enforce CSRF protection (anti-CSRF token and/or `SameSite`)
- Request binding uses explicit field allowlists — no direct binding of request bodies to objects with sensitive fields
- Redirect/forward destinations are server-mapped or validated against a trusted-domain allowlist
...
- **Container runtime hardening**: Where components are containerized, containers MUST run as a non-root user (`USER` set), drop all Linux capabilities and add back only those required, use a read-only root filesystem where feasible, set `no-new-privileges`, and MUST NOT run as `--privileged` or mount the container runtime socket (e.g., `/var/run/docker.sock`)
...
- Container images/manifests run as non-root, drop capabilities, and do not use `--privileged` or mount the runtime socket (N/A for non-containerized components)
...
- **Session ID integrity**: Session identifiers MUST be generated with a CSPRNG (≥128 bits of entropy), be opaque, and carry no embedded data. The session ID MUST be regenerated on any privilege change (login, password change, role elevation) to prevent session fixation, and server-supplied IDs are the only ones accepted
...
- Session IDs are CSPRNG-generated, opaque, and regenerated on login/privilege change (no session fixation)
...
- **XML parser hardening**: Any component that parses XML (including SVG, SAML, DOCX/XLSX, SOAP) MUST disable DTD processing and external entity resolution to prevent XXE, and set entity-expansion/size/depth limits to prevent entity-expansion DoS ("billion laughs")
...
- XML parsers are configured with DTD/external-entity resolution disabled and expansion limits set
...
## Rule SECURITY-16: Cryptographic Standards and Certificate Validation

**Rule**: All cryptographic operations MUST use modern, vetted algorithms and manage keys safely. This rule governs *how* cryptography is implemented, complementing SECURITY-01 (which governs *that* data is encrypted).

- **Banned algorithms**: NEVER use broken or obsolete primitives — MD2/MD4/MD5/SHA-0/SHA-1 for security purposes, DES/3DES/RC2/RC4/Blowfish, AES-ECB, RSA with PKCS#1 v1.5 encryption padding, static RSA or anonymous Diffie-Hellman key exchange.
- **Approved algorithms**:
  - Symmetric: AES-GCM or ChaCha20-Poly1305 (authenticated encryption). CBC/CTR only with encrypt-then-MAC.
  - Asymmetric: RSA ≥ 2048-bit or modern ECC (Curve25519/Ed25519); use OAEP for RSA encryption.
  - Hashing (integrity): SHA-256 or stronger.
  - Password hashing: adaptive, memory-hard algorithms (Argon2id, scrypt, bcrypt, or PBKDF2 with a high iteration count) with a unique per-user salt — never a plain hash. (Cross-references SECURITY-12.)
- **Randomness**: All security-sensitive random values (tokens, session IDs, salts, IVs/nonces, keys) MUST come from a cryptographically secure PRNG (e.g., `SecureRandom`, `crypto.randomBytes`, `secrets`) — NEVER a general-purpose RNG (`Math.random`, `rand()`).
- **Key management**: Keys MUST be generated within, and stored in, a managed key service, HSM, or vault — never hardcoded, derived from predictable inputs, or committed to source. Separate keys by purpose (encryption, signing, wrapping), wrap data keys with a key-encryption key, and support rotation.
- **Certificate validation**: TLS/certificate verification MUST NOT be disabled (no `verify=False`, `rejectUnauthorized: false`, or trust-all trust managers). When the application loads or validates X.509 certificates, it MUST check expiry, hostname/SAN match, key strength (RSA ≥ 2048), and signature algorithm (reject SHA-1-signed certs).

**Verification**:
- No banned hash, cipher, or key-exchange algorithm appears in the codebase or configuration
- Authenticated encryption (AES-GCM / ChaCha20-Poly1305) is used for application-level symmetric encryption
- All tokens, salts, IVs, and keys are generated with a CSPRNG — no general-purpose RNG in security-sensitive paths
- Encryption/signing keys are sourced from a KMS/HSM/vault and are not hardcoded or checked into source control
- TLS certificate verification is never disabled; certificate-loading code validates expiry, hostname, key strength, and signature algorithm

---

## Enforcement Integration

These rules are cross-cutting constraints that apply to every aire stage. At each stage:
...
| SECURITY-16 | A04:2025 – Cryptographic Failures |
