...
**Error**: Existing `aire-state.md` is corrupted
...
**Error**: Cannot determine number of services
...
**Error**: Story dependencies are circular
- **Cause**: Poor boundary definition, tight coupling
- **Solution**: Identify circular `requires` edges, suggest refactoring the stories
- **Recovery**: Re-run the Dependency Graph stage to break cycles

**Error**: Design plan has missing steps
...
- **Cause**: Missing design inputs, unclear requirements
- **Solution**: Return to Requirements Analysis / Application Design to clarify scope
...
**Error**: Story dependencies not satisfied (Doability Gate failed)
- **Cause**: Prerequisite stories (from the story's `requires`) are not yet `🧪 Ready for Testing` in the Story Tracker
- **Solution**: Implement a currently ready story first (see the Doability Gate in `workflows/dev-implement.md` and `implementation/story-selection.md`)
- **Do Not Proceed**: Never bypass the Doability Gate — a story is doable only when ALL its `requires` are `🧪 Ready for Testing`

**Error**: `dev-implement` invoked but no Dependency Graph / Story Tracker exists
- **Cause**: User Stories or Dependency Graph stage was not completed
- **Solution**: Run/complete User Stories (Story Tracker) and the Dependency Graph stage first, then retry `dev-implement`
...
**Scenario**: `aire-state.md` is corrupted or inconsistent

**Recovery Steps**:
1. Create backup: `aire-state.md.backup`
...
3. Reset stage status in `aire-state.md`
...
3. Mark stage as "SKIPPED" in `aire-state.md`
...
- **Solution**: 
  ...
  2. Check if stage was marked complete in aire-state.md
...
**Error**: aire-state.md shows stage complete but artifacts don't exist
- **Cause**: State file updated but artifact generation failed
- **Solution**:
  1. Mark stage as incomplete in aire-state.md
...
1. **Always validate state**: Check aire-state.md matches actual artifacts
