...
1. `.spec/aire-docs/aire-state.md` — the `## Story Tracker` and `## Tracker`.
2. `.spec/aire-docs/planning/user-stories/stories.md` — story details and acceptance criteria.
3. `.spec/aire-docs/planning/requirements/requirements.md` and
   `.spec/aire-docs/planning/requirements/epic-brief.md`.
4. `.spec/aire-docs/implementation/design/` design artifacts, if they exist.
5. `.spec/aire-docs/planning/reverse-engineering/`, if present — for the system's **external
   interfaces** (endpoints, ports, payload shapes).
6. `.spec/aire-docs/tests/` — story folders already generated by earlier `/ve-implement` runs.
7. `## Branching` and `Workflow Type` in `.spec/aire-docs/aire-state.md` — needed in Step 3 to resolve
...
`.spec/aire-docs/tests/<TICKET-ID>-<title>/` already exists, say so and ask whether to **refresh**
...
Read `Workflow Type` in `.spec/aire-docs/aire-state.md` `## Tracker`:
...
- If the resolved single story already has `.spec/aire-docs/tests/<TICKET-ID>-<title>/`, the existing
...
1. Read `Workflow Type` and `## Branching` in `.spec/aire-docs/aire-state.md` to resolve the
...
3. Cut the ve branch from it: **`ve/<TICKET-ID>-<title-kebab>`** (or
   ...
   - **Refreshing existing artifacts**: if Step 2 already found `.spec/aire-docs/tests/<TICKET-ID>-…/`
...
1. **Load `.aire-rule-details/implementation/test-plan.md` and execute it in full**, scoped to
   ...
   - the output folder `.spec/aire-docs/tests/<TICKET-ID>-<title-kebab>/`,
...
1. Stage only this story's new/changed files under
   `.spec/aire-docs/tests/<TICKET-ID>-<title-kebab>/`.
...
5. Record the returned PR URL **and the labels applied** (`ai-generated` + `aire-v[N]`) in the
   `audit.md` entry (`test-plan.md` Step 7) as this story's **ve PR** — do **NOT** write it to
...
once approved, `test-plan.md` Step 7 logs the run in `audit.md`. Then close with the handoff
...
➡️ NEXT ACTIONS — do these in order:
   1️⃣  Merge the ve PR into `<integration-branch>`: <ve PR URL>
   ...
   2️⃣  WAIT to EXECUTE the manual test steps — build and run the system from `<integration-branch>` locally,
       ...
       Test plan: `.spec/aire-docs/tests/<TICKET-ID>-<title-kebab>/`
   3️⃣  Meanwhile — type `/ve-implement <next story>` for the next story
...
7. **One story per run**, into its own `.spec/aire-docs/tests/<TICKET-ID>-<title-kebab>/` folder
...
9. **NEVER modify application code**, and never write anything into a `tests/` directory at the
   workspace root — ve artifacts live only under `.spec/aire-docs/tests/`.
...
11. **Do NOT write the Story Tracker's `PR`/`Merged`/`Status` columns.** The only state this produces
   is the `audit.md` entry defined in `test-plan.md` Step 7 (which records this agent's own
